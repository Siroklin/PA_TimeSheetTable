...
Начиная с версии с авторизацией, в `.env` нужна переменная `SECRET_KEY` (см. `.env.example`) —
если её нет, добавьте перед обновлением:
```bash
echo "SECRET_KEY=$(openssl rand -hex 32)" >> .env
```
При первом запуске после обновления автоматически создастся пользователь `admin`/`admin` —
сразу смените ему пароль через «Сервис → Управление пользователями».
